class Metronome{
    ...
    constructor(audioManager){
        ...
        function scheduleNote( beatNumber, time) {
            ...
            that.tatum++;
            if (beatNumber % (that.numerator*4) === 0){    // beat 0 == high pitch
                if (that.click){
                    if (that.makeSound){
                ...
                // viglis
                if (that.bar >= stop_bar && continuous_play==false){ //if (continuous_play==true) then keep metronome clicking
            ...
            }else if (beatNumber % (that.numerator*4) === 0 ){    // quarter notes = medium pitch
                if (that.click){
                    if (that.makeSound){
                        that.bigClick.currentTime = 0;
                        that.bigClick.play();
        ...
        function scheduler() {
            // trigger time event
            that.timeEvent.metroTimeStamp = that.audioManager.getCurrentTime() - that.startTime;
            that.beatTimeEvent.metroBeatTimeStamp = that.timeEvent.metroTimeStamp*(that.tempo/60.);
